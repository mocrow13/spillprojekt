...
class FRUKT:
    def __init__(self,bildesti):
        self.x = randint(0,cellenumber-10)# fordi på grunn av random, kan posisjonen til eple bli dannet utenfor spillareat 
        self.y = randint(0,cellenumber-10) 
        self.pos = Vector2(self.x,self.y) 
        self.bilde = pygame.image.load(bildesti).convert_alpha()
        self.randomisere()
        
    def tegne_frukt(self):
        eple_rect = pygame.Rect(int(self.pos.x * cellestørrelse),int(self.pos.y*cellestørrelse),cellestørrelse,cellestørrelse) # (20*40))800,800det den koden gjør å plassere fruktene kordinaten til 5 ganges med cellestørrelse og 4 gang cellestørrelse fra toppen  
        ...
        # grape_rect = pygame.Rect(int(self.pos.x * cellestørrelse),int(self.pos.y*cellestørrelse),cellestørrelse,cellestørrelse)
        # vindu.blit(eple,eple_rect)
        vindu.blit(self.bilde, eple_rect)

        # vindu.blit(grape,grape_rect)
...
class Druer(FRUKT):
    def __init__(self):
        super().__init__("grapes.png")
        self.bilde = pygame.transform.scale(self.bilde,(cellestørrelse, cellestørrelse))
...
class MAIN:
    ...
    def tegne_ting(self):
        self.draw_grass()
        self.epler.tegne_frukt()
        self.druer.tegne_frukt()
        self.slange.tegne_slange()
        self.draw_score()
        
    def skjekke_kollisjon(self):
        if self.epler.pos == self.slange.body[0]:
            self.epler.randomisere()
            self.slange.add_blokk()
            self.score += 1
        if self.druer.pos == self.slange.body[0]:
            self.druer.randomisere()
            self.slange.add_blokk()
            self.score += 5
...
eple = pygame.image.load('apple.png').convert_alpha()
grape = pygame.image.load('grapes.png').convert_alpha()
# grape = pygame.image.load('tile.png').convert_alpha()

main_game = MAIN()
SNAKE_UPDATE_TIMER = pygame.USEREVENT + 1
pygame.time.set_timer(SNAKE_UPDATE_TIMER, 150)

DRUER_UPDATE_TIMER = pygame.USEREVENT + 2
pygame.time.set_timer(DRUER_UPDATE_TIMER, 1500)
while True:
    for event in pygame.event.get():
        if event.type== pygame.QUIT:
            pygame.quit()
            exit() #video system not initialized ends any types of code that runs (closes the game without error) 
        if event.type == SNAKE_UPDATE_TIMER:
            main_game.slange.flytte_slange()
        
        if event.type == DRUER_UPDATE_TIMER:
            main_game.druer.flytt()
